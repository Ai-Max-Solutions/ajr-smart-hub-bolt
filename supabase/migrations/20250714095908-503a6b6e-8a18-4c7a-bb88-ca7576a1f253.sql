-- Legendary Smart Notifications System Database Migration
...
CREATE INDEX idx_smart_notifications_priority_created ON public.smart_notifications(priority, created_at) WHERE expires_at > now();
...
CREATE INDEX idx_ai_predictions_active ON public.ai_compliance_predictions(prediction_type, confidence_score) WHERE expires_at > now();
CREATE INDEX idx_voice_logs_recent ON public.voice_interaction_logs(user_id, created_at) WHERE created_at > (now() - interval '7 days');
...
-- Create materialized view for notification performance analytics
CREATE MATERIALIZED VIEW public.notification_performance_summary AS
SELECT 
    ...
    COUNT(*) FILTER (WHERE action_taken IS NOT NULL) as action_count,
    AVG(EXTRACT(EPOCH FROM (read_at - created_at))/60) as avg_read_time_minutes,
    AVG(predicted_compliance_risk) as avg_risk_score
FROM public.smart_notifications
...
CREATE INDEX idx_notification_performance_day_type ON public.notification_performance_summary(day, notification_type);

-- Function to refresh performance analytics
CREATE OR REPLACE FUNCTION public.refresh_notification_analytics()
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY public.notification_performance_summary;
END;
$$;