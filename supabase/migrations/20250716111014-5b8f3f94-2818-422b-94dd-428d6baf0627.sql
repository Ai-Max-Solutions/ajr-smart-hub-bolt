-- Targeted Database Cleanup - Essential fixes only
-- This migration addresses primary key naming and adds essential indexes

BEGIN;

-- Step 1: Rename primary keys from whalesync_postgres_id to id
DO $$
BEGIN
    -- Users table
...
CREATE INDEX IF NOT EXISTS idx_blocks_project ON public."Blocks"(project);
CREATE INDEX IF NOT EXISTS idx_levels_block ON public."Levels"(block);
...
CREATE INDEX IF NOT EXISTS idx_users_currentproject ON public."Users"(currentproject);
...
CREATE INDEX IF NOT EXISTS idx_plots_status ON public."Plots"(plotstatus);
