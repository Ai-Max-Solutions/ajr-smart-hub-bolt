...
-- Create performance analytics function
...
BEGIN
    ...
    WITH project_metrics AS (
        ...
        WHERE wth.plot_id IN (
            SELECT p.whalesync_postgres_id 
            FROM public."Plots" p 
            WHERE p.level IN (
                SELECT l.whalesync_postgres_id 
                FROM public."Levels" l 
                WHERE l.block IN (
                    SELECT b.whalesync_postgres_id 
...
DECLARE
    ...
    v_avg_daily_progress NUMERIC;
    v_remaining_work NUMERIC;
    v_estimated_days NUMERIC;
BEGIN
    WITH recent_progress AS (
        SELECT 
            AVG(completion_percentage) as current_completion,
            COUNT(*) as total_plots,
    ...
    daily_progress AS (
        SELECT 
            work_date,
            COUNT(DISTINCT plot_id) as plots_worked
        FROM public."Work_Tracking_History" wth
        WHERE wth.plot_id IN (
            SELECT p.whalesync_postgres_id 
            FROM public."Plots" p 
...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'efficiency_distribution', (
            SELECT jsonb_agg(
                jsonb_build_object(
                    'efficiency_range', CASE 
                        WHEN plots_per_hour >= 2.0 THEN 'excellent'
                        WHEN plots_per_hour >= 1.5 THEN 'good'
                        WHEN plots_per_hour >= 1.0 THEN 'average'
                        ELSE 'below_average'
                    END,
                    'count', count
                )
