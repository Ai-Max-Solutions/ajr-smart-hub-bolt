...
-- 4. Create secure user profile update policy that prevents role changes
...
WITH CHECK (
  ...
  AND (
    -- Allow all non-role updates by checking if role hasn't changed
    (SELECT role FROM public.users WHERE supabase_auth_id = auth.uid()) = NEW.role
    OR 
...
-- 5. Create role change audit trigger function
...
-- Create trigger for role change auditing (fixed version)
...
-- 6. Fix contractor_rams_signatures RLS conflicts
