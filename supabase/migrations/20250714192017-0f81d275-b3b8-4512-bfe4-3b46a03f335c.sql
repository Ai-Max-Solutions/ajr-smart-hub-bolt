...
-- Make critical user_id fields non-nullable and add role validation constraints

-- 1. Add check constraints for role validation
ALTER TABLE public."Users" 
ADD CONSTRAINT valid_roles 
CHECK (role IN ('Admin', 'Document Controller', 'Project Manager', 'Supervisor', 'Foreman', 'Staff', 'Operative', 'Director'));

ALTER TABLE public."Users" 
ADD CONSTRAINT valid_system_roles 
CHECK (system_role IN ('Admin', 'Document Controller', 'Project Manager', 'Worker', 'Director'));
...
-- 6. Add indexes for better performance on frequently queried fields
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_activity_metrics_user_action ON public.activity_metrics(user_id, action_type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_plot_assignments_user_status ON public."Plot_Assignments"(user_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_work_tracking_user_date ON public."Work_Tracking_History"(user_id, work_date);

-- 7. Add function to prevent role escalation through direct updates
...
BEGIN
    ...
    -- Prevent non-admins from changing roles to admin levels
...
-- Apply the role validation trigger
...
-- 8. Create function to clean up orphaned records
CREATE OR REPLACE FUNCTION public.cleanup_orphaned_records()
RETURNS INTEGER AS $$
DECLARE
    cleanup_count INTEGER := 0;
BEGIN
    -- Clean up activity metrics without valid users
    ...
    -- Clean up plot assignments without valid users or plots
    ...
    -- Clean up work tracking without valid users or plots
