...
WITH CHECK (
  auth.uid() = supabase_auth_id AND
  -- Prevent users from modifying critical security fields
  OLD.role = NEW.role AND
  OLD.supabase_auth_id = NEW.supabase_auth_id AND
  OLD.id = NEW.id
);
...
-- 6. Fix database function search paths for security
...
-- 7. Add role change validation trigger
