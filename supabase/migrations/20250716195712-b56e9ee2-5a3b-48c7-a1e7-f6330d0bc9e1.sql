...
SELECT 
    ...
    CASE 
        ...
        WHEN ROW_NUMBER() OVER() % 3 = 0 THEN 'Submitted'
        ELSE 'Draft'
    END::timesheet_status_enum
FROM sample_data;

-- Create timesheet entries for each timesheet
WITH timesheet_data AS (
    SELECT 
        t.id as timesheet_id,
        pl.id as plot_id,
        wc.id as work_category_id,
        ROW_NUMBER() OVER(PARTITION BY t.id) as entry_num
    FROM public.timesheets t
    CROSS JOIN public.plots pl
    CROSS JOIN public.work_categories wc
    WHERE pl.project_id = t.project_id
    AND ROW_NUMBER() OVER(PARTITION BY t.id) <= 10
)
...
SELECT 
    ...
    CASE entry_num % 3
        WHEN 0 THEN 'Completed installation work as planned'
        WHEN 1 THEN 'Some delays due to material delivery'
        ELSE 'Work progressing well, on schedule'
    END
FROM timesheet_data;