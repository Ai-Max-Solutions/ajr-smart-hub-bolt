-- AJ Ryan SmartWork Hub - Final Setup and Improvements
-- This creates missing tables and optimizes the existing setup
...
DO $$ BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'documents' AND policyname = 'Managers can manage documents') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'site_notices' AND policyname = 'Managers can manage notices') THEN
        CREATE POLICY "Managers can manage notices" ON public.site_notices
            FOR ALL USING (
                EXISTS (
                    SELECT 1 FROM public."Users" 
                    WHERE supabase_auth_id = auth.uid() 
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'on_hire' AND policyname = 'Managers can manage equipment') THEN
        CREATE POLICY "Managers can manage equipment" ON public.on_hire
            FOR ALL USING (
                EXISTS (
                    SELECT 1 FROM public."Users" 
                    WHERE supabase_auth_id = auth.uid() 
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'suppliers' AND policyname = 'Admins can manage suppliers') THEN
...
-- Link users to projects via project_team (with correct role mapping)
WITH projects AS (
...
users AS (
    SELECT whalesync_postgres_id, email, role FROM public."Users" WHERE email IN ('pm1@ajryan.co.uk', 'sup1@ajryan.co.uk', 'op1@ajryan.co.uk', 'op2@ajryan.co.uk')
)
...
FROM projects p
CROSS JOIN users u
WHERE (p.projectname = 'Woodberry Down Phase 2' AND u.email IN ('pm1@ajryan.co.uk', 'sup1@ajryan.co.uk', 'op1@ajryan.co.uk', 'op2@ajryan.co.uk'))
ON CONFLICT DO NOTHING;
...
-- Create sample timesheets with correct lowercase status
...
SELECT 
    ...
    CURRENT_DATE::date,
    37.5,
    'submitted',  -- lowercase as required by constraint
